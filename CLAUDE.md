...
Install / deploy (`scripts/` + `other/`):
- `scripts/install.sh` — server-side install. Builds from source, creates the `rmfakecloud` system user, installs to `/opt/rmfakecloud/bin/`, templates a hardened systemd unit + env file from `other/rmfakecloud.service` and `other/rmfakecloud.env`, starts the service, verifies `/health`, and writes a `device.env` for the companion script. Idempotent: re-running preserves the existing `JWT_SECRET_KEY` unless `--rotate-jwt` is passed. Auto-injects `AmbientCapabilities=CAP_NET_BIND_SERVICE` when `--port < 1024`. Flags: `--storage-url`, `--data-dir`, `--port`, `--prefix`, `--service-user`, `--env-file`, `--unit-path`, `--device-env-out`, `--skip-build`, `--reconfigure`, `--rotate-jwt`, `--verify-only`, `--non-interactive`, `--dry-run`.
- `scripts/device-setup.sh` — operator-workstation companion. Reads `device.env`, ssh-probes `10.11.99.1`, detects rm1/rm2/Paper Pro from `/sys/devices/soc0/machine`, scp's the matching pinned upstream installer from `ddvk/rmfakecloud-proxy` (`installer-rm12.sh` or `installer-rmpro.sh`) plus optional CA cert to the tablet, runs it with `STORAGE_URL` set, verifies `/health` from the device side. Thin wrapper — does not itself edit `/etc/hosts` or install certs.
- `scripts/lib/common.sh` — shared helpers (`log/info/warn/die`, `require_root`, `has_cmd`, `ver_ge`, `write_if_changed`, `run`, `prompt`, `step`). Sourced by both scripts. Honors `DRY_RUN` and `NON_INTERACTIVE`.
- `scripts/device/automagic.sh` is the legacy one-liner that pulls the upstream v0.0.4 installer; kept working but marked deprecated in favor of `device-setup.sh`.
- `other/rmfakecloud.service` and `other/rmfakecloud.env` are the **templates** that `install.sh` substitutes into. The unit is hardened (`NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `MemoryDenyWriteExecute`, `SystemCallFilter=@system-service ~@privileged @resources`, empty `CapabilityBoundingSet`, restricted address families). The env file enumerates every supported `RM_*`/`RMAPI_*` setting grouped by subsystem with source-of-truth references.
...
- `scripts/install.sh` is contract-bearing in two ways: (1) re-runs must preserve `JWT_SECRET_KEY` (rotating it invalidates every issued JWT — both device tokens and web sessions) — only `--rotate-jwt` may rewrite it; (2) the systemd unit it writes is the hardened template from `other/rmfakecloud.service`. Loosening `ProtectSystem`/`ReadWritePaths`/`SystemCallFilter`/`CapabilityBoundingSet` casually will break the service or silently weaken sandboxing — make changes in the template, not by editing the deployed unit.
- `scripts/device-setup.sh` pulls the device-side installer from the external `ddvk/rmfakecloud-proxy` repo at a **pinned** release tag (not floated). When bumping the pin, re-test on each supported model (rm1/rm2/Paper Pro) — the two installer variants diverge.
